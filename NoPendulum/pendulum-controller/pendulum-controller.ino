...
// Tiempo de muestreo 
const float SAMPLE_TIME = 0.002; // 4ms sampling time
...
const float SETPOINT = 180.0; // Posición invertida del péndulo
...
// Configuración del Stepper (precision)
...
/******************************************************************************
* Maquina de estados para el inicio del control
*/
void run_control_loop() {
  ...
  // Solo aplicar control si está en STATE_CONTROL Y en rango
  ...
  // Detener motor si está fuera de control activo
...
void loop() {
  ...
  // Ejecuta siempre el bucle de control si esta en control
  ...
  if (rx_code == ControlComms::OK) {
    ...
    observation[0] = theta; // ángulo del pendulo
    observation[1] = phi; // ángulo del rotor
    observation[2] = dtheta; // velocidad del pendulo
    observation[3] = dphi; // velocidad del rotor
